...
<main class="max-w-4xl mx-auto py-8" role="main" aria-labelledby="page-title">
  ...
  <header class="mb-6">
    <h1 id="page-title" class="text-xl font-semibold text-gray-900">Crear objetivo estratégico</h1>
    <p class="text-sm text-gray-500 mt-1">
      Completa la información del objetivo y guarda los cambios.
    </p>
  </header>
  ...
  <section class="bg-white border border-gray-200 rounded-xl shadow-sm p-6" aria-label="Formulario crear objetivo">
    <form action="{{ route('objetivos.store') }}" method="POST" class="space-y-6">
      @csrf
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        @isset($pnds)
        <div>
          <label for="pnd_id" class="block text-sm font-medium text-gray-700 mb-1">PND</label>
          <select id="pnd_id" name="pnd_id"
                  class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm bg-white
                         focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
            <option value="">— Seleccione —</option>
            @foreach ($pnds as $id => $nombre)
              <option value="{{ $id }}" {{ (string) old('pnd_id') === (string) $id ? 'selected' : '' }}>
                {{ $nombre }}
              </option>
            @endforeach
          </select>
        ...
        @isset($ods)
        <div>
          <label for="ods_id" class="block text-sm font-medium text-gray-700 mb-1">ODS</label>
          <select id="ods_id" name="ods_id"
                  class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm bg-white
                         focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
            <option value="">— Seleccione —</option>
            @foreach ($ods as $id => $nombre)
              <option value="{{ $id }}" {{ (string) old('ods_id') === (string) $id ? 'selected' : '' }}>
                {{ $nombre }}
              </option>
            @endforeach
          </select>
      ...
      <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
        <a href="{{ route('objetivos.index') }}" class="btn-secondary">
          Volver
        </a>
