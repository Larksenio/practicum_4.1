...
<div class="max-w-5xl mx-auto py-8">
    ...
    {{-- Header --}}
    <div class="mb-5">
        <h1 class="text-xl font-semibold text-gray-900">Editar reporte</h1>
        <p class="text-sm text-gray-500 mt-1">
            Actualiza la información del reporte y guarda los cambios.
        </p>
    </div>
    ...
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm p-6">
        <form action="{{ route('reportes.update', $reporte) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select name="tipo"
                        ...
                        @foreach (['PDF', 'Excel', 'Web'] as $t)
                            <option value="{{ $t }}"
                                {{ old('tipo', $reporte->tipo) == $t ? 'selected' : '' }}>
                                {{ $t }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="responsable_id"
                        ...
                        @foreach ($usuarios as $id => $nom)
                            <option value="{{ $id }}"
                                {{ old('responsable_id', $reporte->responsable_id) == $id ? 'selected' : '' }}>
                                {{ $nom }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <textarea name="antecedentes" rows="5"
                              class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm
                                     focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">{{ old('antecedentes', $reporte->antecedentes) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="desarrollo" rows="5"
                              class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm
                                     focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">{{ old('desarrollo', $reporte->desarrollo) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="conclusiones" rows="5"
                              class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm
                                     focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">{{ old('conclusiones', $reporte->conclusiones) }}</textarea>
                </div>
            </div>
